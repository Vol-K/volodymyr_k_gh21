...
def ajax_add_to_cart(request):
    ...
    # Getting all nedded info from request.
    request_body = request.body
    dict_from_request_body = json.loads(request_body.decode('utf-8'))
    prod_id = dict_from_request_body["prod_id"]
    prod_category = dict_from_request_body["category"]

    # Added one more product to the cart (all logic).
    if prod_id:
        added = added_processing(request, prod_id, prod_category)
        print("added", added)
    return JsonResponse({"product_quantity": added[0], "cart_quantity": added[1]})


# All operations for rendering a user cart page.
