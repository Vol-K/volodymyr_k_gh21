...
def index():
    ...
    if session.get("user_id") is not None:
        ...
        if request.method == "POST":
            ...
            # чекаємо базу для додаткової інфи, на майбутнє
            check_matches = db.execute ("SELECT match_id, round, mtch_round, match_date, match_time\
                                         FROM m3 WHERE razom =:razom", dict(razom = inp_razom))
            check_matches = check_matches.fetchone()

            # витягуємо id матчу 
            match_id = check_matches[0]
            tur_nomer = check_matches[1] # номер тура
            match_round = check_matches[2] # матч в турі

            # ДЛЯ НЕДОПУЩЕННЯ ДУБЛІКАТА ПРОГНОЗА
            # чекаємо прогнози користувача 
            check_duplicate = db.execute("SELECT teams FROM p3 WHERE user_id = :user_id AND match_id = :match_id",\
                                          dict(user_id=session["user_id"], match_id = match_id))
            check_duplicate = check_duplicate.fetchone()

            # якщо такий запис вже є, то редірект заново на сторінку ставок
            ...
            # перевіряємо базу на наявність двох типів прогнозів (про всяк випадок), і якщо є кидаємо помилку
            ...
            # чекаємо актуальний час на момент прогноза (без мілісекунд)
            date_and_time_now = datetime.now()
            date_and_time_now = date_and_time_now.replace(microsecond=0)

            # коригуємо часовий пояс (додаємо +2/3 години) 
            # !треба переводити на Зимовий/Літній
            date_and_time_now_timestamp = datetime.timestamp(date_and_time_now)
            date_and_time_now_timestamp = date_and_time_now_timestamp + 7200    ###60 сек * 60 хв * 2 години - Зимовий час
            date_and_time_now_timestamp = date_and_time_now_timestamp + 10800    ###60 сек * 60 хв * 3 години - Літній час

            ## час для внесення прогноза в базу
            date_and_time = date_and_time_now_timestamp

            ### блокуємо ставки після початку матча
            # витягуємо день та час матчу
            match_day = check_matches[3]
            match_time = check_matches[4]

            ## об'єднуємо день та час матчу в один рядок
            ...
            # конвертуємо 'str' в нормальний час, згідно заданого формата і далі переганяємо у формат 'datetime.datetime' 
            # для порівняння з актуальним часом прогноза чи зміни прогноза
            match_day_time_timestamp = datetime.timestamp(datetime.strptime(match_day_time, '%Y-%m-%d %H:%M'))

            ## звіряємо час початку матчу з актувальним часом прогноза
            delta_chas = match_day_time_timestamp - date_and_time_now_timestamp

            if delta_chas < 0:
                return apology("Chas vyishov...", 500)
            ...
            if inp_prognoz_type == 'exp':
                ...
                add_prognoz = db.execute ("INSERT INTO p3 (oper_id, match_id, user_id, teams, home, guest, tur, status, time, mtch_round, prognoz_type)\
                                        ...
                                        guest=inp_prognoz_visitors, tur=tur_nomer, time=date_and_time, mtch_round=match_round, prognoz_type='Експрес'))
                dbb.commit()

            # якщо прогноз - звичайний (ординар)
            if inp_prognoz_type == 'ord':
                # пишемо прогноз в базу
                add_prognoz = db.execute ("INSERT INTO p3 (oper_id, match_id, user_id, teams, home, guest, tur, status, time, mtch_round, prognoz_type)\
                                        VALUES (NULL, :match_id, :user_id, :teams, :home, :guest, :tur, 'new', :time, :mtch_round, :prognoz_type)",\
                                        dict(match_id=match_id, user_id=session["user_id"], teams=inp_razom, home=inp_prognoz_home,\
                                        guest=inp_prognoz_visitors, tur=tur_nomer, time=date_and_time, mtch_round=match_round, prognoz_type='Ординар'))
                dbb.commit()
        ...
        # Перша відкриття сторінки, і якщо пререйшли просто по ссилі
            ...
            dostupni_matchi_start = db.execute("SELECT razom FROM m3 WHERE availability = 'yes' AND razom NOT IN\
                                               (SELECT teams FROM p3 WHERE user_id =:user_id)",\
                                                dict(user_id = session["user_id"]))
...
#            return apology("Use min 3 number in pass", 400)
        ...
        if request.form.get("admin") == 'yes':
            ...
            if db_check_username == None:
                ...
                user_id_check = db.execute("SELECT user_id FROM users WHERE user_name =:username",\
                                            dict(username=inp_username))
                dbb.commit()
                ...
                print("user_id_check", user_id_check)
                ...
                ft = db.execute("SELECT * FROM fintab")
                #dbb.commit()
                #ft = ft.fetchone()
                print(ft)
...
""" ГОТОВО - вхід користувача / адміна """
...
def changeprognoz():
    ...
    if request.method == "POST":
        ...
        # чекаємо id матчу, дату та час, і витягуємо їх з бібліотеки
        check_match_info = db.execute ("SELECT match_id, match_date, match_time FROM m3 WHERE razom =:razom",\
                                        dict(razom = inp_razom))
        check_match_info = check_match_info.fetchone()

        match_id_only = check_match_info[0]
        # витягуємо день та час матчу
        match_day = check_match_info[1]
        match_time = check_match_info[2]
        
        ## БЛОКУЄМО бідь-які ДІЇ з матчами після початку самого матча
        # чекаємо актуальний час на момент прогноза (без мілісекунд)
        date_and_time_now = datetime.now()
        date_and_time_now = date_and_time_now.replace(microsecond=0)
        ...
        # КОРИГУВАННЯ часу треба тільки для сервера, на локальному хості все ок
        date_and_time_now = date_and_time_now + 10800                        ###60 сек * 60 хв * 3 години

        # перетворюємо актуальний час в 'timestamp' для порівняння з днем-часом матча
        date_and_time_now_timestamp = datetime.timestamp(datetime.fromtimestamp(date_and_time_now))
        ...
        # вивід (редірект) сторінки з доступними матчами для прогноза та зробленими прогнозами від юзверя
        return redirect("/")
...
def zbros():
    ...
    if request.method == "POST":
        ...
        # скидаємо дані по підсумковій табличці
...
def vidkryty_tur():
    ...
    if request.method == "POST":
        ...
        ## частина кода яка треба при ТЕСТУВАННІ, якщо заново відкриваємо Перший тур
...
def del_player_from_team():
    ...
    if request.method == "POST":
        ...
        inp_user_id = request.form.get("gridRadios")

        # і видаляємо його
        del_user = db.execute("UPDATE fintab SET team = NULL WHERE user_id = :user_id",\
                               dict(user_id=inp_user_id))
        dbb.commit()

        # рендерим сторінку
        return redirect ("del_player_from_team")

    else:
        # команди та їх учасники
        check_list = db.execute("SELECT user_id, user_name, team FROM fintab WHERE team NOT NULL ORDER BY team DESC")
        check_list = check_list.fetchall()
